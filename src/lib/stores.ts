...
export const useGamificationStore = create<GamificationProps>()((set) => ({
  ...
  shirtColor: "#ec1010",
  setShirtColor: (newShirtColor) => set(() => ({ shirtColor: newShirtColor })),
}));
...
export const useGameStore = create<GameStateProps>()((set) => ({
  ...
  decreaseShotClock: () =>
    set((state) => {
      if (state.shotClock > 0) {
        return { shotClock: state.shotClock - 1 };
      } else {
        return { shotClock: 0 };
      }
    }),
  ...
  tournamentRound: 1,
  setTournamentRound: (newRound) => set(() => ({ tournamentRound: newRound })),
